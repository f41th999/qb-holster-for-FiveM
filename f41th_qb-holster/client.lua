...
PlayerData = xPlayer -- Update the PlayerData in QBCore
end)

RegisterNetEvent('QBCore:Client:OnJobUptade')
AddEventHandler('QBCore:Client:OnJobUptade',  function(job)
  QBCore.Functions.GetPlayerData().job.name = job
end)
...
function drawWeaponLarge(ped, newWeapon)
    ...
    if playerJob == 'police' and (door == 'driver' or door == 'passenger') then
        blocked = true
        local coordA = GetEntityCoords(ped, 1)
        local coordB = GetOffsetFromEntityInWorldCoords(ped, 0.0, 2.0, 0.0)
        local vehicle = QBCore.Functions.GetClosestVehicle(coordA)
    ...
    elseif not isNearTrunk() then
        ...
        local coordA = GetEntityCoords(ped, 1)
        local coordB = GetOffsetFromEntityInWorldCoords(ped, 0.0, 2.0, 0.0)
        local vehicle = GetClosestVehicle(coordA)
        if DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) then
...
function rackWeapon()
	local door = isNearDoor()
	if PlayerData.job == 'police' and (door == 'driver' or door == 'passenger') then
		blocked = true
		local coordA = GetEntityCoords(ped, 1)
		local coordB = GetOffsetFromEntityInWorldCoords(ped, 0.0, 2.0, 0.0)
